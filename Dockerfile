...
ENV UID=1000 GID=1000 \
  ...
  ENABLE_AUTOPAUSE=false AUTOPAUSE_TIMEOUT_EST=3600 AUTOPAUSE_TIMEOUT_KN=120 AUTOPAUSE_TIMEOUT_INIT=600 AUTOPAUSE_PERIOD=10 AUTOPAUSE_KNOCK_INTERFACE=eth0 \
  ENABLE_AUTOSHUTDOWN=false AUTOSHUTDOWN_TIMEOUT_EST=120 AUTOSHUTDOWN_TIMEOUT_INIT=600 AUTOSHUTDOWN_PERIOD=10

COPY start* /
COPY health.sh /
ADD files/autopause /autopause
ADD files/autoshutdown /autoshutdown

RUN dos2unix /start* && chmod +x /start*
RUN dos2unix /health.sh && chmod +x /health.sh
RUN dos2unix /autopause/* && chmod +x /autopause/*.sh
RUN dos2unix /autoshutdown/* && chmod +x /autoshutdown/*.sh
